...
function switchTab(id) {
    ...
    document.querySelectorAll('.charRels_tab-content').forEach(content => content.classList.remove('active'));
...
function initRelationshipChart(orgColor, charRelL) {
    ...
    window.nodes.forEach(node => {
        ...
        img.style.borderColor = node.data.borderColer || "var(--charRelsChartCharAvatar-borderColor)";
...
function resizeStageToFitNodes() {
    ...
    // 1. 先強制依照最後的物理座標畫一次圖
    // 這樣才能讓 drawFrame 算出那些動態擴張的 .org-box 的真實寬高與位置
    drawFrame();

    // 2. 測量所有頭像與組織框的「真實極限邊界」
    ...
    window.nodes.forEach(node => {
        ...
        if (node.y + 60 > maxY) maxY = node.y + 60; // +60 包含名字的高度
    });

    // 再抓取剛剛 drawFrame 畫出來的「組織框 (org-box)」極限值
    document.querySelectorAll('.org-box').forEach(box => {
        ...
        // 如果加上標籤 (label) 有超出，也可以在這裡微調，但通常 box 邊界已足夠
    ...
    // 設定安全邊距 (只要一點點就好，因為已經測量了真實邊界)
    const safePadding = 30; 
    
    let requiredWidth = (maxX - minX) + (safePadding * 2);
    let requiredHeight = (maxY - minY) + (safePadding * 2);
    ...
    // 計算偏移量，把最左上角的元素推回正數座標 (確保不會被裁切)
    const offsetX = safePadding - minX;
    const offsetY = safePadding - minY;
    ...
    function drawFrame() {
        ...
        orgColor.forEach((orgData, orgIndex) => {
            ...
            orgBox.style.backgroundColor = orgData.bg;

            const label = document.createElement("div");
            label.className = "org-label";
            label.style.backgroundColor = orgData.borderColer;
            label.textContent = orgData.org;
            label.style.color = orgData.color || "var(--charRelsChartorgData-font)";
...
function initCharacterDatabase(charRelL) {
    ...
    charRelL.forEach(char => {
        ...
        const img = document.createElement("img");
        img.src = char.url;
        img.style.borderColor = char.borderColer || "var(--charRelsChartCharAvatar-borderColor)";
        ...
        bRight.innerHTML = `<div class="detail-placeholder">請點擊中間的組織或角色檢視詳細檔案</div>`;
        ...
        html += `<div style="margin:5px;">
                    <div class="liking-container">
                        ...
                        <div class="liking-bar-bg">
                            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; z-index: 2; font-size: 12px; color: var(--characterLikingBar-font); text-shadow: var(--characterLikingBar-textShadow); pointer-events: none;">
                                ${rawLiking}</div>
